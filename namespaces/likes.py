...
likes_request_model = Model(
    "Likes model",
    {
        "user_1": fields.String(required=True, description="Id of the user who liked"),
        "user_2": fields.String(required=True, description="Id of the user who was liked")
    },
)
...
class Likes(Resource):
    ...
    def post(self):
        ...
        like = Like.query.filter(Like.user_1 == id_1 and Like.user_2 == id_2).first()
        if like is None:
            like = Like.query.filter(Like.user_2 == id_1 and Like.user_1 == id_2).first()
            if like is None:
                new_like = Like(args)
